...
class ProblemService(
  ...
  fun matchWithBaekjoonProblem(
    ...
    companyAlgoExpectation.forEach {
      val selectedTwoProblem =
          baekjoonProblemService.matchBaekjoon(it.problemLevel, it.problemAlgorithm)
      finalProblemInfoList.add(ProblemDto.revertBaekjoonProblemDto(selectedTwoProblem))
    }
  ...
  fun makeCompanyExpectTest(company: Company): List<ProblemDto.ProblemInfo> {
    ...
    problemList.add(chooseFirstProblemInfo(company.companyProblems?.find { it.problemNumber == 1 }))

    for (i in 2..company.problemNumber) {
      problemList.add(
          chooseNextAlgorithmBasedOnHistory(
              problemList,
              company.companyProblems?.find { it.problemNumber == i }
  ...
  fun chooseRandomLevel(problemExceptLevel: Float): Int {
    ...
    return if (Random.nextFloat() <= upPercent) problemExceptLevel.toInt() + 1
    else problemExceptLevel.toInt()
  }
  ...
  fun chooseNextAlgorithmBasedOnHistory(
    ...
    val problemLevel = companyProblem?.problemLevel ?: throw ProblemNotFoundException()
    val beforeChosenAlgo = beforeProblemList.map { it.problemAlgorithm }
    ...
    val confirmLevel = chooseRandomLevel(problemLevel)
    val confirmAlgorithm =
        if (problemAlgoMap.isEmpty()) {
          val algoList = AlgoWeightEnum.getEnumList()
          val selectableMap = algoList.filterNot { beforeChosenAlgo.contains(it.name) }
          AlgoWeightEnum.getRandomEnumByWeight(selectableMap)
        } else {
          chooseRandomAlgorithm(problemAlgoMap)
        }

    return ProblemDto.ProblemInfo(problemAlgorithm = confirmAlgorithm, problemLevel = confirmLevel)
  }
}
