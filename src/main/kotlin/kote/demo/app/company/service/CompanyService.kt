package kote.demo.app.company.service

import java.nio.file.Files
import java.nio.file.Path
import java.nio.file.Paths
import kote.demo.app.company.dto.CompanyInfoDto
...
class CompanyService(
  ...
  fun getCompanyList(): List<String> {
    return companyRepository.findAll().map { it.name }
  }
  ...
  fun saveNewCompany(companyInfo: CompanyInfoDto.CompanyRequest, file: MultipartFile) {
    ...
    if (existingCompany != null) {
      //            throw CompanyAlreadyExistsException("Company '${companyInfo.companyName}'
      // already exists")
    ...
    for (num in 1..companyInfo.problemCount) {
      companyProblemList.add(
          CompanyProblem(company = company, problemNumber = num, problemAlgo = ""))
    }
  ...
  fun patchProblemPreference(problemAlgo: CompanyInfoDto.ProblemPreferRequestDto) {
    val company =
        companyRepository.findByName(problemAlgo.companyName) ?: throw CompanyNotFoundException()
    val companyProblemList =
        companyProblemRepository.findAllByCompanyOrderByProblemNumberAsc(company)

    updateProblemLevel(
        problemAlgo.problemNumber,
        companyProblemList,
        RatingEnum.valueOf(problemAlgo.problemLevel).level.toFloat())
  ...
  fun updateProblemLevel(
    ...
    if (number > 1) {
      if (companyProblemList[number - 2].problemLevel != null) {
        companyProblemList[number - 2].problemLevel =
            (companyProblemList[number - 2].problemLevel!! + 0.1f * level) / 1.1f
      }
    }

    if (number < companyProblemList.size) {
      if (companyProblemList[number].problemLevel != null) {
        companyProblemList[number].problemLevel =
            (companyProblemList[number].problemLevel!! + 0.1f * level) / 1.1f
      }
    }
  }
